...
class ErgoNetSingleProcessor:
    def __init__(self, device):
        ...
        model_path = self.root_folder_path + '/checkpoints/model_ckpt_385000.pth'
        ...
        self.predictor = self.get_model(self.ckpt_path, smpl_mean_path)
        self.predictor.eval()

        self.PoseEstimator2D = VitInference(vitpose_path, model_name='h', is_video=False, device=device)

    def update_model_weights(self, model_path):
        """
        ...
        """
        self.ckpt_path = model_path
        self.predictor = self.get_model(self.ckpt_path, self.smpl_mean_path)
        self.predictor.eval()
    ...
    def optimize_joint(self, theta, beta, cam_full, camera_matrix,
                       smplx_model, body_j2d, body_j2d_conf, hand_kp2d, hand_kp2d_conf, num_iter=1500):
        """
    ...
    def process_input(self,img,camera_intrinsics,poi=None,poi_type='bbox'):
        """
        ...
        raw_bboxes = np.concatenate(
            [detection_results.xyxy[detection_results.class_id == 1], detection_results.confidence[detection_results.class_id == 1][:, None]],
            axis=1)
        if len(raw_bboxes) == 0:
            print("no person is being detected.")
        ...
        if poi != None:
            ...
            for i in range(len(raw_bboxes)):
                ...
                if poi_type == 'bbox':
                    x1, y1, x2, y2 = max(raw_bbox[0], poi[0]), max(raw_bbox[1], poi[1]), min(
                        raw_bbox[2], poi[2]), min(raw_bbox[3], poi[3])
        ...
        all_crop_ul = []
        all_crop_br = []
        print('number of people:', len(bboxes_xywh))
        ...
        norm_img_final = torch.from_numpy(all_norm_img).float().to(self.device)
        center_final = torch.from_numpy(all_center).float().to(self.device)
        scale_final = torch.from_numpy(all_scale).float().to(self.device)
        crop_ul_final = torch.from_numpy(all_crop_ul).float().to(self.device)
        crop_br_final = torch.from_numpy(all_crop_br).float().to(self.device)
        bboxes_final = torch.from_numpy(bboxes_xywh).float().to(self.device)
        body_keypoints2d_final = torch.from_numpy(body_keypoints).float().to(self.device)
        hand_keypoints2d_final = torch.from_numpy(hand_keypoints).float().to(self.device)
        image_shape_final = torch.from_numpy(np.asarray([img_h,img_w])).repeat(bboxes_final.size()[0], 1).float().to(self.device)
        camera_intrinsics_tensor = torch.from_numpy(np.asarray(camera_intrinsics)).repeat(bboxes_final.size()[0], 1).float().to(self.device)

        return norm_img_final, center_final, scale_final,crop_ul_final,crop_br_final,bboxes_final,body_keypoints2d_final,hand_keypoints2d_final,image_shape_final,camera_intrinsics_tensor
    ...
    def check_input(self,img,camera_intrinsics,camera_distortion, gender, poi, poi_type):
        """
        Check if the inputs are in the required format.
        """
        image_check = isinstance(img, np.ndarray) and img.ndim == 3 and img.shape[2] == 3
        ...
        if camera_distortion != None:
            if len(camera_intrinsics) != 5:
                return False
    ...
    def run(self,img,camera_intrinsics=None,camera_distortion=None,
            gender='neutral', poi=None, poi_type='bbox',joint_optimize=True):
        """
        Run the ErgoNet pipeline on the input image and get the results.

        Args:
            img: raw and unprocessed input image
            camera_intrinsics (None if unknown): camera intrinsics parameters (fx, fy, cx, cy)
            ...
            camera_distortion (None if unknown): camera distortion coefficients [k1, k2, p1, p2, k3]
            ...
            joint_optimize: Optimize ErgoNet's predicted results using ViTPose's joint predictions (True / False)

        Returns:
            results: a dictionary includes the predicted results of ErgoNet
        """
        if self.check_input(img,camera_intrinsics,camera_distortion, gender, poi, poi_type) is False:
        ...
        if camera_intrinsics is None:
            ...
            camera_intrinsics = [focal_length, focal_length, raw_img_w / 2, raw_img_h / 2]
        if camera_distortion is not None:
            camera_matrix = np.asarray([[camera_intrinsics[0], 0, camera_intrinsics[2]],
                                         [0, camera_intrinsics[1], camera_intrinsics[3]],
                                         [0, 0, 1]])
            img = cv2.undistort(img,camera_matrix,camera_distortion)

        with torch.no_grad():
            norm_img, center, scale,crop_ul,crop_br,bboxes,body_keypoints2d,hand_keypoints2d,image_shape,camera_intrinsics_tensor = self.process_input(img,camera_intrinsics,poi,poi_type)
            if norm_img is None:
                return None
